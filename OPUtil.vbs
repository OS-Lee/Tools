...
Dim fNeedGenericSql, fRemovePatchQnD, fXl, fRemovePatchInstalledBefore, fRemovePatchInstalledAfter
...
Dim sOSinfo, sOSVersion, sComputerName, sExternalMsi, sViewScope, sRemovePatchInstalledAfter
Dim sRemovePatchInstalledBefore

Dim iIndex, iVersionNt, iMspSizeTotal, iMspRemoveCnt
...
If (fApplyPatch AND NOT fViewPatch) OR fCleanCache OR fForceRemovePatch OR fReconcileCache OR fRemovePatch OR fRemovePatchInstalledAfter OR fRemovePatchInstalledBefore OR fRepairCache Then fShowLog = True
...
If fRemovePatchInstalledAfter Then 
    Log Space(30) & "/RemovePatchInstalledAfter=" & sRemovePatchInstalledAfter
End If
If fRemovePatchInstalledBefore Then 
    Log Space(30) & "/RemovePatchInstalledBefore=" & sRemovePatchInstalledBefore
End If
...
sTmp = "Initialize Restore Locations "
...
sTmp = "Initialize Update Locations "
Log vbCrLf & vbCrLf & sTmp & vbCrLf & String(Len(sTmp), "-")
...
'-------------------------------
If fRemovePatch OR fRemovePatchInstalledAfter OR fRemovePatchInstalledBefore Then 
    Dim arrMspRemove
    Dim Msp, Item
    ...
    If fRemovePatch Then
        For Each Item In arrMspRemove
            ...
            End If
        Next 'Item
        If InStr(sMspRemoveFilter, "{") > 0 Then
            sTmp = "Running MspRemove with filter: '" & sMspRemoveFilter & "'"
            Log vbCrLf & vbCrLf & sTmp & vbCrLf & String(Len(sTmp), "-")
            MspRemove sMspRemoveFilter, sMspProductFilter
        End If
        If InStr(UCase(sMspRemoveFilter), "SUPERSEDED") > 0 Then
            sTmp = "Running MspRemove with filter: '" & sMspRemoveFilter & "'"
            Log vbCrLf & vbCrLf & sTmp & vbCrLf & String(Len(sTmp), "-")
            MspRemove "Superseded", sMspProductFilter
        End If
    End If
    If fRemovePatchInstalledAfter OR fRemovePatchInstalledBefore Then 
        sTmp = "Running MspRemove with timescope filter:" 
        If fRemovePatchInstalledAfter Then sTmp = sTmp & " 'Installed after " & sRemovePatchInstalledAfter & "'"
        If fRemovePatchInstalledBefore Then sTmp = sTmp & " 'Installed before " & sRemovePatchInstalledAfter & "'"
        Log vbCrLf & vbCrLf & sTmp & vbCrLf & String(Len(sTmp), "-")
        MspRemove "", sMspProductFilter
    End If
End If
...
If iMspSizeTotal > 0 Then
    sLogSummary = sLogSummary & vbCrLf & "Total count of uninstallable patches: " & iMspRemoveCnt
    sLogSummary = sLogSummary & vbCrLf & "Total size of uninstallable patches: " & CLng(iMspSizeTotal / 1024^2) & " MB" & vbCrlf
End If
...
'-------------------------------------------------------------------------------
Sub Initialize()
    Dim Item, ComputerItem, Process, Processes, DateTime, qOS, oOsItem
    Dim iInstanceCnt
    Dim arrVersion
    ...
    fRemovePatchInstalledAfter = False
    fRemovePatchInstalledBefore = False
    iMspSizeTotal = 0
    iMspRemoveCnt = 0
    ...
    'Get properties from this computer
...
Sub ParseCmdLine()
    ...
    '        fApplyPatch = True
        ...
        Case "/REMOVEPATCHINSTALLEDAFTER", "-REMOVEPATCHINSTALLEDAFTER", "REMOVEPATCHINSTALLEDAFTER"
            fValidCmdFound = True
            fShowLog = True
            If fArray Then
                fRemovePatchInstalledAfter = True
                sRemovePatchInstalledAfter = arrArg(1)
                iActionCnt = iActionCnt + 1
                fArray = False
            End If
    
        Case "/REMOVEPATCHINSTALLEDBEFORE", "-REMOVEPATCHINSTALLEDBEFORE", "REMOVEPATCHINSTALLEDBEFORE"
            fValidCmdFound = True
            fShowLog = True
            If fArray Then
                fRemovePatchInstalledBefore = True
                sRemovePatchInstalledBefore = arrArg(1)
                iActionCnt = iActionCnt + 1
                fArray = False
            End If
...
Sub InitRepairDic()
    ...
    sTmp = "Get content from package locations"
    Log vbCrLf & vbCrLf & sTmp & vbCrLf & String(Len(sTmp), "-")
    ...
    For Each Folder in arrRestoreLocations
        ...
        Log vbTab & "Collect files from Restore location: " & Folder
        For Each File in oFso.GetFolder(Folder).Files
            ...
            Case ".msp"
                ...
                If Not dicRepair.Exists(sKey) Then
                    Log vbTab & "Add file from restore location. Key: " & sKey & " - " & File.Path
                    dicRepair.Add sKey, File.Path
                End If
            Case ".msi"
    ...
    For Each Product in oMsi.Products
        ...
        If NOT fMsiOK Then
            ...
            Else
                ...
                Log vbTab & "Check Repair Dic for: " & sKey
                If dicRepair.Exists(sKey) Then 
                    ...
                    End If 'fDetectOnly
                Else
                    Log vbTab & "No match found in Repair Dic."
                    fTrySource = True
                End If
                ...
                If fTrySource Then
                    ...
                    For Each Source in MsiSources
                        Log vbTab & "Check MSI resiliency location: " & Source
                        ...
                        If oFso.FileExists(sFile) Then
                            ...
                            If sKey = sSourceKey Then
                                If Not dicRepair.Exists(sSourceKey) Then
                                    Log vbTab & "Add to Repair Dic: " & sSourceKey & " - " & sFile
                                    dicRepair.Add sSourceKey, sFile
                                End If
                            ...
                            Else
                                Log vbTab & "No match found in resiliency source: " & Source
                ...
                For Each Patch in PatchList
                    ...
                    If NOT sLocalMsp = "" Then
                        ...
                        Else
                            ...
                            Log vbTab & "Local patch package missing. File: " & sLocalMsp & ", PatchCode: " & sKey
                            Log vbTab & "Debug: Check Repair Dic for: " & sKey
                            If dicRepair.Exists(sKey) Then
                                Log vbTab & "Debug: Found match in Repair Dic."
                                'Debug
                                Log vbTab & "Debug: Matching file from Repair Dic: " & dicRepair.Item(sKey)
                                Log vbTab & "Debug: Check access to repair file: " & oFso.FileExists(dicRepair.Item(sKey))
                                If NOT fDetectOnly Then
                                    sRetVal = oFso.CopyFile(dicRepair.Item(sKey), sLocalMsp)
                                    Log vbTab & "Debug: CopyFile returned: " & sRetVal
                                End If
                                If oFso.FileExists(sLocalMsp) Then
                                    sRepair = "Restored: Successfully connected to 'RestoreLocation' (" & dicRepair.Item(sKey) & ") to restore local .msp package '"
                                Else
                                    fTrySource = True
                                    'Handle 'DetectOnly' exception
                                    If fDetectOnly Then
                                    ...
                                    Else
                                        Log vbTab & "Error: FileCopy failed to copy file '" &  dicRepair.Item(sKey) & "' to targetfile '" & sLocalMsp & "'"
                                    End If 'fDetectOnly
                                End If
                            Else
                                Log vbTab & "Debug: No match found in Repair Dic."
                                fTrySource = True
                            End If
                            ...
                            If fTrySource Then
                                ...
                                For Each Source in MspSources
                                    Log vbTab & "Debug: Trying to connect to resiliency source " & Source
                                    If fRepaired Then Exit For
                                    sFile = Source & sPackage
                                    If oFso.FileExists(sFile) Then
                                        sSourceKey = ""
                ...
                If NOT sLocalMsp = "" Then
                    ...
                    Else
                        ...
                        If fTrySource Then
                            ...
                            For Each Source in MspSources
                                If fRepaired Then Exit For
                                Log vbTab & "Check MSP resiliency location: " & Source
                                sFile = Source & sPackage
                                If oFso.FileExists(sFile) Then
                                    sSourceKey = ""
                                    sSourceKey = oMsi.SummaryInformation(sFile, MSIOPENDATABASEMODE_READONLY).Property(PID_REVNUMBER)
                                    If sKey = sSourceKey Then
                                        If NOT dicRepair.Exists(sSourceKey) Then
                                            Log vbTab & "Add file from resiliency location. Key: " & sKey & " - " & File.Path
                                            dicRepair.Add sSourceKey, sFile
                                        End If
                                        If NOT fDetectOnly Then oFso.CopyFile sFile, sLocalMsp
                                        fRepaired = oFso.FileExists(sLocalMsp)
...
'   MspReconcile
'
'   Unregister .msp files that have gone missing or corrupt in the 
'   '%windir%\installer' folder
'-------------------------------------------------------------------------------
Sub MspReconcile()
    Const MAX_ATTEMPT = 100

    Dim Prod, Product, Patch, PatchList, ProductsList, Msp, SumInfo
    Dim sLocalMsp, sLocalMsi
    Dim iCnt
    Dim fResume, fMspOk, fMspUnregister
                ...
                If Err = 0 Then
                    For Each Patch in PatchList
                        ...
                        If NOT oFso.FileExists(sLocalMsp) Then 
                            ...
                            If NOT fDetectOnly Then Exit For 'For Each Patch in PatchList
                        ...
                        Else
                            ...
                            'Ensure SummaryInformation table can be accessed
                            Set Msp = oMsi.OpenDatabase (sLocalMsp, MSIOPENDATABASEMODE_PATCHFILE)
                            Set SumInfo = Msp.SummaryInformation
                            If Not Err = 0 Then
                                'An error at this points indicates a severe issue. Flag msp for removal
                                fMspUnregister = True
                                sTmp = vbTab & "Error: Local patch package '" & sLocalMsp & "' corrupt (SummaryInformation table) '" & Patch.PatchCode & "' - '" & Patch.PatchProperty("DisplayName") & "'. Unregistering patch ..."

                            End If

                            If fMspUnregister Then
                                'Call patch unregister routine
                                If NOT dicMspUnregister.Exists(Patch.PatchCode) Then dicMspUnregister.Add Patch.PatchCode, Patch.PatchCode
                                iCnt = iCnt + 1
                                If iCnt < MAX_ATTEMPT Then fResume = True
                                If NOT fDetectOnly Then UnregisterPatch Patch
                                'Refresh PatchesEx object and resume
                                If NOT fDetectOnly Then Exit For 'Patch in PatchList
                                'Reset flag for detect only case
                                fResume = False
                                fMspOk = False
                            End If
...
'-------------------------------------------------------------------------------
Sub ApplyPatches()
    Dim Product, Patch, PatchList, Key
    Dim sPatchFile, sReturn, sAbsent, sAppliedPatches
    Dim iIndex
    Dim fFeatureControl
    ...
    CollectSUpdates
    If IsArray(arrSUpdatesAll) Then
        Log vbTab & "Debug:  Found " & UBound (arrSUpdatesAll) + 1 & " unique patch(es) in total." & vbCrLf
...
Sub CollectSUpdates()
    ...
    For Each Folder in arrUpdateLocations
        If fCscript Then wscript.echo vbTab & "Collect files from " & Folder
        Log vbTab & "Collect msp files from folder: " & Folder
        For Each File in oFso.GetFolder(Folder).Files
                ...
                If LCase(File.Path)=LCase(sApplyPatch) OR sApplyPatch = "" OR (LCase(oFso.GetFolder(Folder).Path) & "\" = LCase(sWICacheDir)) Then
                    ...
                    If Not dicSUpdatesAll.Exists(sKey) Then
                        ...
                        If IsOfficePatch(sPatchTargets) Then
                            Log vbTab & "Add Office msp to patch dic. Key: " & sKey & " - " & File.Path
            ...
            End If '.msp
        Next 'File
        Log vbTab & "Debug:  Found " & dicSUpdatesAll.Count - iCnt & " unique patch(es) in folder " & Folder
...
'-------------------------------------------------------------------------------
Sub GetRawBuckets(sProductCode, sAppliedPatches)
	Dim Patch
	Dim sApplicablePatches, sProductVersion, sDetectedVersion
	Dim iBucket, iIndex
    ...
    If IsArray(arrSUpdatesAll) Then
        For iIndex = 0 To UBound(arrSUpdatesAll)
            ...
            If ((NOT fExcludeCache) OR (NOT fIsWICached)) AND _
			    ...
			    ' exclude patches that do not target the product
                If (InStr(arrSUpdatesAll(iIndex, COL_TARGETS), sProductCode) > 0) Then
			    	...
			    	' patch targets the current product
                    Select Case GetMspBucket(sProductCode, iIndex)
                    Case MSP_NOSEQ
                        If NOT dicMspNoSeq.Exists(arrSUpdatesAll(iIndex, COL_PATCHCODE)) Then dicMspNoSeq.Add arrSUpdatesAll(iIndex, COL_PATCHCODE), iIndex
                    Case MSP_NOBASE
						If Not dicMspNoBase.Exists(arrSUpdatesAll(iIndex, COL_PATCHCODE)) Then  dicMspNoBase.Add arrSUpdatesAll(iIndex, COL_PATCHCODE), iIndex
					Case MSP_MINOR
                        If Not dicMspMinor.Exists(arrSUpdatesAll(iIndex, COL_PATCHCODE)) Then  dicMspMinor.Add arrSUpdatesAll(iIndex, COL_PATCHCODE), iIndex
                    Case MSP_SMALL
                        If NOT dicMspSmall.Exists(arrSUpdatesAll(iIndex, COL_PATCHCODE)) Then  dicMspSmall.Add arrSUpdatesAll(iIndex, COL_PATCHCODE), iIndex
                    Case Else
                    End Select
                Else
...
'-------------------------------------------------------------------------------
Sub SequenceMspMinor(sProductCode, sAppliedPatches)
        ...
        For iCntBld = 0 To UBound(arrMspUpdatedVersions)
            For Each Key in dicMspMinor.Keys
                ...
                If IsValidVersion (sProductCode, arrSUpdatesAll (iIndex, COL_PATCHXML), sProductVersionNew, sErr, iPos) Then
                    ...
                    arrErr = Split(sErr, ";", 2)
                    'Check if patch is already installed to this product
                    If (NOT InStr(sAppliedPatches, Key) > 0) Then
                        'Update applicable reference counter
    ...
    For Each Key in dicMspMinor.Keys
        If NOT InStr(sMspApplicable, Key) > 0 Then
            'patch excluded because higher or matching baseline has been found
            iIndex = dicMspMinor.Item(Key)
            'Update reference counter
            If InStr(sAppliedPatches, Key) > 0 Then
                arrSUpdatesAll(iIndex, COL_APPLIEDCNT) = arrSUpdatesAll(iIndex, COL_APPLIEDCNT) & sProductCode & ";"
                sTmp = "Excluding Service Pack patch KB " & arrSUpdatesAll(iIndex, COL_KB) & " because it's already installed for this product." & _
            ...
            Else
                ...
                sTmp = "Excluding Service Pack patch KB " & arrSUpdatesAll(iIndex, COL_KB) & " because it's superseded by a later service pack." & _
                       " Patch details: " & arrSUpdatesAll(iIndex, COL_PATCHCODE) & ", " & arrSUpdatesAll(iIndex, COL_PACKAGE) & ", " & arrSUpdatesAll(iIndex, COL_FILENAME)
                If dicMspMinor.Exists(Key) Then dicMspMinor.Remove Key
            End If
            Log vbTab & "Debug:  " & sTmp
...
'-------------------------------------------------------------------------------
Sub SequenceMspNoSeq(sProductCode, sAppliedPatches)
    Dim Key, Patch, PatchList
    Dim sErr
    Dim iIndex, iPos
    ...
    For Each Key in dicMspNoSeq.Keys
        iIndex = dicMspNoSeq.Item(Key)
        ' remove patch if already installed
        If (InStr(sAppliedPatches, Key) > 0) Then
            ' update applied reference counter
            arrSUpdatesAll(iIndex, COL_APPLIEDCNT) = arrSUpdatesAll(iIndex, COL_APPLIEDCNT) & sProductCode & ";"
            If NOT Left(arrSUpdatesAll(iIndex, COL_FILENAME), Len(sWICacheDir)) = sWICacheDir Then
                sTmp = "Patch KB " & arrSUpdatesAll(iIndex, COL_KB) & " is already installed." & _
                       " Patch details: " & arrSUpdatesAll(iIndex, COL_PATCHCODE) & ". " & arrSUpdatesAll(iIndex, COL_PACKAGE) & ", " & arrSUpdatesAll(iIndex, COL_FILENAME)
                Log vbTab & "Debug:  " & sTmp
                LogSummary sProductCode, vbTab & sTmp
            End If
        ...
        If dicMspObsoleted.Exists(Key) Then
            ' update superseded reference counter
            arrSUpdatesAll(iIndex, COL_SUPERSEDEDCNT) = arrSUpdatesAll(iIndex, COL_SUPERSEDEDCNT) & sProductCode & ";"
            If NOT Left(arrSUpdatesAll(iIndex, COL_FILENAME), Len(sWICacheDir)) = sWICacheDir Then
...
'-------------------------------------------------------------------------------
Sub SequenceMspNoBase (sProductCode, sAppliedPatches)
	Dim Key, Element, Elements
	Dim sMspApplicable, sFamily, sSeq, sErr, sAttr
	Dim iIndex
	Dim fApplicable, fInstalled
	...
	For Each Key in dicMspNoBase.Keys
		fApplicable = False
		iIndex = dicMspNoBase.Item(Key)
        fInstalled = InStr (sAppliedPatches, Key) > 0
		...
		If NOT fApplicable Then
			If fInstalled Then
                'patch excluded because it's already installed and superseded
			    'Update superseded reference counter
			    arrSUpdatesAll(iIndex, COL_SUPERSEDEDCNT) = arrSUpdatesAll(iIndex, COL_SUPERSEDEDCNT) & sProductCode & ";"
			    sTmp = "Patch KB " & arrSUpdatesAll(iIndex, COL_KB) & " is already installed. It's superseded by a later patch of the same family." & _
				       " Patch details: " & arrSUpdatesAll(iIndex, COL_PATCHCODE) & ", " & arrSUpdatesAll(iIndex, COL_PACKAGE) & ", " & arrSUpdatesAll(iIndex, COL_FILENAME) & _
		...
		Else
            If fInstalled Then
                'Patch is already installed add to applied reference counter
			    arrSUpdatesAll(iIndex, COL_APPLIEDCNT) = arrSUpdatesAll(iIndex, COL_APPLIEDCNT) & sProductCode & ";"
			    sTmp = "Patch KB " & arrSUpdatesAll(iIndex, COL_KB) & " is already installed." & _
				       " Patch details: " & arrSUpdatesAll(iIndex, COL_PATCHCODE) & ", " & arrSUpdatesAll(iIndex, COL_PACKAGE) & ", " & arrSUpdatesAll(iIndex, COL_FILENAME) & _
				       vbCrLf & vbTab & vbTab & "Patch build: " & sSeq & ". Installed build: " & dicMspSequence.Item(sFamily)
                If dicMspNoBase.Exists(Key) Then dicMspNoBase.Remove Key
            Else
                'Patch is applicable and not installed
                'Update applicable reference counter
			    arrSUpdatesAll(iIndex, COL_APPLICABLECNT) = arrSUpdatesAll(iIndex, COL_APPLICABLECNT) & sProductCode & ";"
			    sTmp = "Found applicable baselineless patch with sequence version " & sSeq & " : KB " & arrSUpdatesAll(iIndex, COL_KB) & _
                       " Patch details: " & arrSUpdatesAll(iIndex, COL_PATCHCODE) & ", " & arrSUpdatesAll(iIndex, COL_PACKAGE) & ", " & arrSUpdatesAll(iIndex, COL_FILENAME) & _
				       vbCrLf & vbTab & vbTab & "Patch build: " & sSeq
                       If dicMspSequence.Exists(sFamily) Then sTmp = sTmp & ". Installed build: " & dicMspSequence.Item(sFamily)
            End If
		End If
		Log vbTab & "Debug:  " & sTmp
		LogSummary sProductCode, vbTab & sTmp
...
'-------------------------------------------------------------------------------
Sub SequenceMspSmall(sProductCode, sAppliedPatches)
	Dim Key, Element, Elements
	Dim sMspApplicable, sFamily, sSeq, sErr, sAttr
	Dim iIndex, iPos
	Dim fApplicable, fInstalled
	...
	For Each Key in dicMspSmall.Keys
		...
		Else
			...
			Else
                ...
                'Update invalid baseline reference counter
				arrSUpdatesAll(iIndex, COL_NOQALBASELINECNT) = arrSUpdatesAll(iIndex, COL_NOQALBASELINECNT) & sProductCode & ";"
	...
	For Each Key in dicMspSmall.Keys
		...
		If NOT fApplicable Then
            ...
            Else
			    ...
			    'Update superseded reference counter
			    arrSUpdatesAll(iIndex, COL_SUPERSEDEDCNT) = arrSUpdatesAll(iIndex, COL_SUPERSEDEDCNT) & sProductCode & ";"
			    sTmp = "Patch KB " & arrSUpdatesAll(iIndex, COL_KB) & " will not be installed. It's superseded by a later patch of the same family." & _
				       " Patch details: " & arrSUpdatesAll(iIndex, COL_PATCHCODE) & ", " & arrSUpdatesAll(iIndex, COL_PACKAGE) & ", " & arrSUpdatesAll(iIndex, COL_FILENAME) & _
				       vbCrLf & vbTab & vbTab & "Patch build: " & sSeq & ". Installed build: " & dicMspSequence.Item(sFamily)
			End If
            If dicMspSmall.Exists(Key) Then dicMspSmall.Remove Key
		Else
            ...
            Else
                ...
                'Update applicable reference counter
			    arrSUpdatesAll(iIndex, COL_APPLICABLECNT) = arrSUpdatesAll(iIndex, COL_APPLICABLECNT) & sProductCode & ";"
			    sTmp = "Found applicable patch with sequence version " & sSeq & " : KB " & arrSUpdatesAll(iIndex, COL_KB) & _
                       " Patch details: " & arrSUpdatesAll(iIndex, COL_PATCHCODE) & ", " & arrSUpdatesAll(iIndex, COL_PACKAGE) & ", " & arrSUpdatesAll(iIndex, COL_FILENAME) & _
				       vbCrLf & vbTab & vbTab & "Patch build: " & sSeq
                       If dicMspSequence.Exists(sFamily) Then sTmp = sTmp & ". Installed build: " & dicMspSequence.Item(sFamily)
            End if
            ...
            If Element.selectSingleNode("TargetProductCode").text = sProductCode Then
                ...
                Else
                    ...
                    sTmp = "No valid baseline available for service pack patch KB " & arrSUpdatesAll(iIndex, COL_KB) & ". This may indicate a BETA ProductVersion." & _
                            " Patch details: " & arrSUpdatesAll(iIndex, COL_PATCHCODE) & ", " & arrSUpdatesAll(iIndex, COL_PACKAGE) & ", " & arrSUpdatesAll(iIndex, COL_FILENAME) & _
                            vbCrLf & vbTab & vbTab & "Patch baseline(s): " & arrErr(1) & ". Installed baseline: " & sProductVersion
                    Log vbTab & "Debug:  " & sTmp
                    LogSummary sProductCode, vbTab & sTmp
                End If 'IsValidVersion
...
Sub AddSequenceData(sXml)
	...
	For Each Element in Elements
		...
		If Element.selectSingleNode("Attributes").text = "1" Then
			If dicMspSequence.Exists(sFamily) Then
				If sSeq > dicMspSequence.Item(sFamily) Then
					dicMspSequence.Item(sFamily) = sSeq
					Log vbTab & "Debug: adding higher version for " & sFamily & " - " & sSeq
				Else
					Log vbTab & "Debug: Keep existing item for " & sFamily & " - " & dicMspSequence.Item(sFamily) & " over incoming version of " & sSeq
				End If
			Else
				dicMspSequence.Add sFamily, sSeq
				Log vbTab & "Debug: Adding new entry for family " & sFamily & " - " & sSeq
			End If
		End If 'Attributes = 1
	Next 'Element
...
Sub MspRemove(sPatchCodes, sProductCodes)
    ...
    Dim sPatches, sCmd, sReturn, sStateFilter, sLogFilter, sMspInstallDate
    Dim iFoo, iActivityCnt
    Dim arrLogFilter
    Dim fPatchLoop, fSupersededMode, fMatchFound, fDateScope
    ...
    End If
    If sLogFilter = "" AND (fRemovePatchInstalledAfter OR fRemovePatchInstalledBefore) Then sLogFilter = "InstalledTimeScope"
                ...
                For iFoo = 1 To 1
                    Do 
                        ...
                        For Each Patch in oPatches
                            ...
                            Else
                                ' Check on date filter
                                If fRemovePatchInstalledAfter OR fRemovePatchInstalledBefore Then
                                    fDateScope = False
                                    sMspInstallDate = ""
                                    sMspInstallDate = Patch.PatchProperty("InstallDate")
                                    If fRemovePatchInstalledAfter Then 
                                        If (sMspInstallDate > sRemovePatchInstalledAfter) Then fDateScope = True Else fDateScope = False
                                    End If
                                    If fRemovePatchInstalledBefore Then 
                                        If (sMspInstallDate < sRemovePatchInstalledBefore) Then fDateScope = True Else fDateScope = False
                                    End If
                                    If fRemovePatchInstalledBefore AND fRemovePatchInstalledAfter Then 
                                        If ((sMspInstallDate > sRemovePatchInstalledAfter) AND (sMspInstallDate < sRemovePatchInstalledBefore)) Then fDateScope = True Else fDateScope = False
                                    End If
                                End If

                                If InStr(sPatchCodes, Patch.PatchCode) > 0 OR fSupersededMode OR fDateScope Then
                                    ...
                                    If bQuiet Then sCmd = Replace(sCmd, "/qb-", "/q")
                                    If Patch.PatchProperty("Uninstallable") = "1" Then
                                        fMatchFound = True
                                        ' track msp file size
                                        If NOT InStr(sPatches, Patch.PatchCode) > 0 Then
                                            If oFso.FileExists(sMspFile) Then
                                                iMspSizeTotal = iMspSizeTotal + oFso.GetFile(sMspFile).Size
                                                iMspRemoveCnt = iMspRemoveCnt + 1
                                            End If
                                        End If
...
Sub DiscoverDynSUpdateFolders
    ...
    Log vbTab & "Checking for optimized update location structure."
...
Function EnsureLocation(sLocations)
    ...
    Log vbTab & "Locations to validate: " & sLocations
    ...
    sLocations = Replace(sLocations, ";;", ";")
    Log vbTab & "Updated locations to validate: " & sLocations
    ...
    For Each Location in arrLocations
        ...
        Log vbTab & "Validate location: " & sLocation
        ...
        If oFso.FolderExists (sLocation) Then
            ...
            Log vbTab & "Success: Validated: " & sLocation
        Else
        	Log vbTab & "Error: Discard invalid location: " & sLocation
        End If
        For Each DynLocation in dicDynCultFolders.Keys
            If oFso.FolderExists (sLocation & DynLocation) Then
                ...
                Log vbTab & "Success: Added optimized location: " & LCase(sLocation) & LCase(DynLocation)
            End If
        Next 'DynLocation
    Next 'Location
    sLocations = LCase(sLocalLocations) & LCase(sNetworkLocations) & ";" & LCase(sWICacheDir)
    Log vbTab & "Locations after validation: " & sLocations
...
Dim sReturn
    ...
    Case 14, 15, 16
        ...
        Case "0032" : sReturn = "WAC LPK"
        Case "0033" 
            If iVM = 14 Then sReturn = "Personal" Else sReturn = "Office PIPC1"
        ...
        Case "005B" : sReturn = "Visio Compatibility Pack"
        ...
        Case "0077" : sReturn = "BizModeler"
        Case "0078" : sReturn = "Outlook Social Connector for FaceBook"
        ...
        Case "00D1"
            If iVM = 14 Then sReturn = "Access Database Engine" Else sReturn = "Access Connectivity Engine ACE"
    ...
    Case 12
        ...
        Case "011F" : sReturn = "Outlook Connector"
        Case "0122" : sReturn = "Outlook Connector"
